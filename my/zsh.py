...
@lru_cache
@listify
def read_prefix(p: Path) -> Sequence[str]:
    """
...
@listify
def filter_inputs(paths: Sequence[Path]) -> Sequence[Path]:
    # if a history file starts with the same 100 lines as another one
    # and one file is bigger than another, keep the bigger file
    sorted_files: List[Path] = sorted(paths, key=lambda p: p.lstat().st_size, reverse=True)
    unique_files: Path = [sorted_files.pop(0)]
