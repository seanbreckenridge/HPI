...
try:
    ...
    try:
        from my.config import common as user_config  # type: ignore[attr-defined, assignment, misc, no-redef]

        warnings.high(
            "'common' config section is deprecated. Please rename it to 'core'."
        )
...
class Config(user_config):
    ...
    # FIXME use appdirs cache dir or something
    cache_dir: Optional[PathIsh] = None  # type: ignore[assignment]

    # list of regexes/globs
    # None means 'rely on disabled_modules'
    enabled_modules: Optional[Sequence[str]] = None  # type: ignore[assignment]
