...
def test_set_repo(tmp_path: Path) -> None:
    pass
    # shouldnt really worry about testing explicit
    # export paths because those are gonna be phased
    # out eventually
    #
    # set_repo itself is being phased out in favor
    # of setuptools

    # from my.cfg import config
    # class user_config:
    #    export_path = 'whatever',
    ## just has to be some module that accepts an export path
    # config.reddit = user_config # type: ignore[misc,assignment]

    ## precondition:
    ## should fail because can't find rexport
    # with pytest.raises(ModuleNotFoundError):
    #    # something that uses the dummy user config and fails
    #    import my.reddit
